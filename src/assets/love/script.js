// src/assets/love/script.js
(function () {

  function init() {
    var canvas = $('#canvas');

    // Si aún no existe el canvas (Angular no renderizó), salimos
    if (!canvas.length) return false;

    // Si ya se inicializó una vez, no repetir (evita duplicados si vuelves a Home)
    if (canvas.data('loveStarted')) return true;
    canvas.data('loveStarted', true);
  ...
  // Exponemos una función global para llamarla desde Angular
  window.startLove = function () {
    // intenta una vez ya
    if (init()) return;

    // si aún no está el canvas, reintenta un ratito
    var tries = 0;
    var timer = setInterval(function () {
      tries++;
      if (init() || tries > 50) clearInterval(timer); // ~5s máximo
    }, 100);
  };
